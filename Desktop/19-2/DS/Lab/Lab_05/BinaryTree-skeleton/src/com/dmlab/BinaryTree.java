...
public class BinaryTree<Key extends Comparable<? super Key>, E> {
	...
	class Node {
		...
		public Node delete(Key key) {
			// TODO: Fill this function
			if (this.find(key) == null) return this;

			int compare = this.mKey.compareTo(key);
			if (compare > 0) {
				this.mLeft = this.mLeft.delete(key);
			} else if (compare < 0) {
				this.mRight = this.mRight.delete(key);
			} else { // This is deleting node
				if (this.mLeft == null && this.mRight == null) {
					return null;
				} else if (this.mLeft == null) { // Only R
					return this.mRight;
				} else if (this.mRight ==null) { // Only L
					return this.mLeft;
				} else { // Deleting node has both L, R
					this.mKey = this.mRight.findMin().getKey();
					this.mValue = this.mRight.findMin().getValue();
					this.mRight = this.mRight.delete(this.mKey);
				}
			}
