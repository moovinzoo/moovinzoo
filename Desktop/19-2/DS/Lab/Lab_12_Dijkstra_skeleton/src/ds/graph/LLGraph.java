...
public class LLGraph implements Graph{
    ...
    public int next(int v, int w) {
        Edge it = null;
        if (isEdge(v, w)) {
            vertex[v].next(); // pos가 쳐다보던 다음친구로 이동해서
            it = vertex[v].getValue(); // Edge 뽑아내고
        }
        if (it != null) {
            return it.vertex(); // vertex로 리턴.
        }

        return vertex.length; // |V|
    }
